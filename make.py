...
for file in all_md_files:
    with open(file,'r') as f:
        ...
        for link in md_links:
            ...
            print(f'md_image_path:: {md_image_path}')
            md_image_filename = os.path.basename(md_image_path)
            print(f'md_image_filename:: {md_image_filename}')
            md_image_title = re.search(r'\[(.*?)\]', link).group(1)
            print(f'md_image_title:: {md_image_title}')

            # Drop the "_files" in filename location [EP]
            image_file_location = md_image_path.replace("_files", "")
            print(f'image_file_location:: {image_file_location}')
            print('=============================================')

            new_link = "![" + md_image_title + "](" + image_file_location + ")"
